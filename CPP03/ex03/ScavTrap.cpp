...
ScavTrap::~ScavTrap()
{
	std::cout << "Done with ScavTrap ";
	std::cout.write(getName().c_str(), getName().length() - 10) << std::endl;
}
...
void 	ScavTrap::attack(const std::string &target)
{
	...
	{
		std::cout.write(getName().c_str(), getName().length() - 10) << " attacks "
...
{
	std::cout.write(getName().c_str(), getName().length() - 10) << " is in Gate keeper mode" << std::endl;
}
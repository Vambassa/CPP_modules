...
ClapTrap::ClapTrap(int hit, int energy, int damage, std::string name) :	name(name), hit(hit), energy(energy), damage(damage)
{
	std::cout << "ClapTrap " << name << " created" << std::endl;
}
...
{
	if (hit <= 0)
		std::cout << "ClapTrap " << target <<  " is already dead! Stop it!" << std::endl;
	else if (!energy)
...
{
	if (hit <= 0)
		std::cout << name << " is already dead! Stop it!" << std::endl;
	else
	{
		hit -= amount;
...
{
	if (hit <= 0)
		std::cout << name << " is already dead, so can't recover." << std::endl;
	else if (!energy)
		std::cout << name
